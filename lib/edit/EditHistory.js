...
Xr.edit.EditHistory = Xr.Class({
	...
	methods: {
	    undo: function(/* optional bool */ bFullUpdate) {
	        ...
	        if (cmd.undo()) {
	            ...
	            if (bFullUpdate) {
	                ...
	                this._editManager.targetGraphicLayer().refresh();
	            }
	        }
	    ...
	    redo: function (/* optional bool */ bFullUpdate) {
		    ...
		    var cmd = this._commands[this._index];
		    if (cmd.run()) {
		        var e = Xr.Events.create(Xr.Events.RedoActionCompleted, {});
		        Xr.Events.fire(this._editManager.map().container(), e);
