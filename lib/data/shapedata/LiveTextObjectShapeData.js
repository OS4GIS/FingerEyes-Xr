...
/**  
 * @classdesc LiveTextObjectGraphicRow에 대한 도형 정보 값을 담고 있는 클래스입니다. 
 * @class
 * @param {Object} arg - 객체이며 x, y, text 속성이 있어야 함. x는 텍스트가 위치할 지도 좌표에 대한 X축 값이며 y는 텍스트가 위치할 지도 좌표에 대한 Y축 값, 그리고 url은 문자열을 얻기 위해 호출하는 url이며 period는 호출 주기입니다.
 * @copyright [(주)지오서비스]{@link http://www.geoservice.co.kr}
 * @license LGPL
 */
Xr.data.LiveTextObjectShapeData = Xr.Class({
    name: "LiveTextObjectShapeData",
    requires: [Xr.data.IShapeData, Xr.edit.ISnap],

    construct: function (/* {x:Number, y:Number, text:String, offsetX:int, offsetY:int, url:String, period:int } */ arg) {
        this._data = arg;
    ...
    methods: {
        ...
        /* ShapeData */ clone: function () {
            ...
            var newThing = new Xr.data.LiveTextObjectShapeData(arg);
            newThing._mbr = mbr;
        ...
        /* int */ type: function () {
            return Xr.data.ShapeType.LIVE_TEXT_OBJECT;
        },
