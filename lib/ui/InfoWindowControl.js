...
Xr.ui.InfoWindowControl = Xr.Class({
    ...
    construct: function (/* String */ name, /* Map */ map, /* PointD */ position, /* string */ innerHtml,
        ...
        if (option) {
            ...
            this._topOnClick = option.topOnClick ? option.topOnClick : false;

            this._borderWidth = option.borderWidth ? option.borderWidth : "3px"; 
        ...
        this._strokeWidth = this._borderWidth ? parseInt(this._borderWidth) : 3;
        ...
        container.appendChild(this._skinSvg);

        if (this._opZoom && this._opZoom != 1) {
            container.style.transform = "scale(" + this._opZoom + ", " + this._opZoom + ")";
        ...
        if (!this._hideCloseButton) {
            ...
            container.appendChild(this._closeBtnSvg);
        ...
        container.addEventListener('mousewheel', this._mouseWheel);
        container.addEventListener('DOMMouseScroll', this._mouseWheel);

        if (this._topOnClick) {
            container.addEventListener("click", function (e) {
                var lastChild = that.map().userControls().container().lastChild;
                var thatChild = container;
