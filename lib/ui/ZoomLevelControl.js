...
/**  
 * @classdesc 줌레벨(Zoom Level)에 대한 설정을 위한 UI 컨트롤에 대한 클래스입니다. 이 컨트롤을 이용해 지정된 지도 축척으로 지도를 확대, 축소할 수 있습니다.
 * @class
	    ...
	    /* DIV */ _createThumb: function () {
            ...
            */
            ...
            that.__onThumbMouseDown = function (e) {
                ...
                if (!that._mouseDown) {
                    ...
                    that._deltaY = e.clientY;
                    that._mouseDown = true;

                    document.addEventListener("mousemove", that.__onThumbMouseMove);
                    document.addEventListener("mouseup", that.__onThumbMouseUp);
                }
            };
            ...
            */
            ...
            that.__onThumbMouseMove = function (e) {
                ...
                if (that._mouseDown) {
                    var proThumbY = that._thumbY + (e.clientY - that._deltaY);
                    if (that._isThumbInAvailableRange(proThumbY)) {
                        that._thumbY += (e.clientY - that._deltaY);
                        div.style.top = (that._thumbY + (55 - that.thumbHeight() / 2)) + "px";
                    }
	    ...
	    update: function () {
            ...
            */
	        ...
	        if (!this.container().style.height || this.container().style.height === "") {
                svg.style.height = this.container().style.height = (this._getBarHeight() + 30 * 2) + "px" ;
                svg.style.width = this.container().style.width = (30 * 2) + "px";
	        }
	    },
