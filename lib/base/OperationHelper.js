...
Xr.OperationHelper = Xr.Class({
    ...
    statics: {
        ...
        uploadFile: function (args, /* Do not specify value! File Seek Position */ start) {
            /*  
            var args = {
                id: 1, 
            ...
            */
            ...
            if (args.bStop) {
             ...
             // 업로드 할 파일이 null일 경우 바로 onCompleted를 호출하여 다음 단계를 처리할 수 있도록 하였음
            if (args.imageFile === null) {
                if (args.onCompleted) {
                    if (args.onProgress) {
                        args.onProgress(args.id, 100);
                    }

                    args.onCompleted(args.id);
                }
                    ...
                    if (xhr.readyState === 4) {
                        if (xhr.status === 200) {
                            ...
                            if (next_slice < file.size) {
                                if (args.onProgress) {
                                    args.onProgress(args.id, next_slice / file.size * 100.0);
                                }
