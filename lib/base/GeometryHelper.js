...
Xr.GeometryHelper = Xr.Class({
    ...
    statics: {
         ...
         */
        /* PointD */ centroidOfPolyline: function (/* PointD Array */ coordsList) {
        ...
        /* float */ lengthOfPolyline: function (/* PointD Array */ coordsList) {
            var cntCoord = coordsList.length;
            var totalLen = 0;

            for (var i = 1; i < cntCoord; i++) {
                var coord1 = coordsList[i - 1];
                var coord2 = coordsList[i];

                totalLen += Math.sqrt((coord1.x - coord2.x) * (coord1.x - coord2.x) + (coord1.y - coord2.y) * (coord1.y - coord2.y));
            }
         ...
         * @return {Xr.PointD} 폴리라인에 대한 라벨 표시 위치
         */
        /* Object {pt: PointD, angle: float} */ centroidOfPolylineWithAngle: function (/* PointD Array */ coordsList) {
            var halfLen = this.lengthOfPolyline(coordsList) / 2;
            var p = this.verticesOnPolylineByDistance(coordsList, halfLen, halfLen);
            ...
            var cntCoord = coordsList.length;
            if (cntCoord > 1) {
                var coord1, coord2, len;
                var halfLen = this.lengthOfPolyline(coordsList) / 2;
                for (var i = 1; i < cntCoord; i++) {
                    coord1 = coordsList[i - 1];
                    coord2 = coordsList[i];
                    len = Math.sqrt((coord1.x - coord2.x) * (coord1.x - coord2.x) + (coord1.y - coord1.y) * (coord1.y - coord1.y));

                    if (halfLen - len <= 0) {
                        var ang = -this.angle(coord1, coord2);
                        ...
                        return { pt: this.vertexOnLineByDistance(coord1, coord2, halfLen), angle: ang };
                    } else {
                        halfLen -= len;
                    }
                }
         ...
         */
        /* number */
        //distanceOfLineFromPoint: function (/* PointD */ p1, /* PointD */  p2, /* PointD */  pt, /* PointD */ itPt) {
        //    var lineMag = Xr.GeometryHelper.magnitude(p2, p1);
        //    var u = (((pt.x - p1.x) * (p2.x - p1.x)) +
        //        ((pt.y - p1.y) * (p2.y - p1.y))) /
        //        (lineMag * lineMag);
        ...
        /* number */ distanceBetweenPointAndPoint: function (/* PointD */ Pt1, /* PointD */ Pt2) {
        ...
        /* PointD */ vertexOnLineByDistance: function(/* PointD */ lineA, /* PointD */ lineB, /* number */ distance)
        {
            ...
            if(t > 1.0) {  
                ...
                return new Xr.PointD(lineA.x + t * (lineB.x - lineA.x), lineA.y + t * (lineB.y - lineA.y));
            }
        },

        /* number */ lengthOfLine: function (/* PointD */ lineA, /* PointD */ lineB) {
            return Math.sqrt((lineA.x-lineB.x)*(lineA.x-lineB.x)+(lineA.y-lineB.y)*(lineA.y-lineB.y));
        },
        
        /* array of object {pt:PointD, angle:number} */ verticesOnPolylineByDistance: function (/* Array Of Xr.PointD */ pts,  /* number */ startDistance, /* number */ distance) {
            var cntPts = pts.length;
            var dist = startDistance;

            if (cntPts > 1) {
                var result = [];
                var vertexOnLineByDistance = Xr.GeometryHelper.vertexOnLineByDistance;
                var lengthOfLine = Xr.GeometryHelper.lengthOfLine;
                var remainLength = 0;

                for (var i = 1; i < cntPts; i++) {
                    var pt1 = pts[i - 1];
                    var pt2 = pts[i];
                    var totalLengthLine = lengthOfLine(pt1, pt2);

                    if (totalLengthLine > (dist - remainLength)) {
                        var ang = -this.angle(pt1, pt2);
                        var absAng = Math.abs(ang);
                        if (absAng >= 90 && absAng <= 270) {
                            ang += 180;
                        }

                        var xpt = vertexOnLineByDistance(pt1, pt2, (dist - remainLength));

                        do {
                            result.push({ pt: xpt.clone(), angle: ang });

                            dist = distance;

                            xpt = vertexOnLineByDistance(xpt, pt2, dist);
                        } while (xpt/* != undefined*/);

                        remainLength = lengthOfLine(result[result.length-1].pt, pt2);
                    } else {
                        remainLength += totalLengthLine;
                    }
                }
